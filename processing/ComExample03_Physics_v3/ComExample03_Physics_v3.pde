...
float myFloatArray[] = new float[numberOfObjects];
...
void setup() {
  ...
  for (int i=0; i<numberOfObjects; i++){
      myImageArray[i] = loadImage( "foto" + i + ".png");
...
void drawString(float x, float size, int cards) {
  ...
  // anchor card
  CustomShape s1 = new CustomShape(x, -40, size, BodyType.DYNAMIC, int (random(0,3)), int (random(10,50)));
  polygons.add(s1);
  ...
  {
    float y = -20 + gap * (i+1);
    next_shape = new CustomShape(x, -20 + gap * (i+1), size, BodyType.DYNAMIC, int (random(0,3)),int (random(10,50)));
...
void updateAndDrawBox2D() {
  ...
  if (frameRate > 24) {
    CustomShape shape1 = new CustomShape(kinectWidth/2, -50, -1,BodyType.DYNAMIC,int (random(0.0,3.0)),int (random(10,50))) ;
    CustomShape shape2 = new CustomShape(kinectWidth/2, -50, random(2.5, 20),BodyType.DYNAMIC,int (random(0.0,3.0)),int (random(10,50)));
    polygons.add(shape1);
    polygons.add(shape2);
  }
