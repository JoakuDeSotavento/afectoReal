...
class CustomShape {
  ...
  CustomShape(float x, float y, float r, BodyType type, int nimagen, int grosor) {
    this.r = r;
    this.nimagen = nimagen;
    //this.grosor = grosor;
    this.grosor = int (r+grosor);
  ...
  void display() {
    ...
    if (r == -1) {
     ...
     int grosora = this.grosor*1;
     image( myImageArray[this.nimagen], 0-grosora/2,0-grosora/2, grosora,grosora);
