...
void updateTracking() {
  ...
  int userCount = context.getNumberOfUsers();   //our test for num# of users
  ...
  if (userCount > 0) {  
    ...
    if (trackSkeleton) {
      ...
      if(context.isTrackingSkeleton(trackUserNum)) {
 ...
 */
 void drawUsersAsPixels() {
  //working with user Pixels, in B&W or color, will decrease frameRate by about 18
    ...
    //cutting out the context.rgbImage() //RGB steps 1-4, increases rate. 
    // no RGB, steps = 3 => frameRate is 12.5 - 13.0
    
    //PImage rgbImage = context.rgbImage();       //RGB3. load the color image from the Kinect
    int[] userMap = null;                        //pixel arrays of users
    depthMap = context.depthMap();                //pixel array of depthMap, set as global 
    PVector[] depthPoints = context.depthMapRealWorld(); //array of realWorldPoints
    int steps = 3;                               //increase step-through of pixel array to decrease CPU load
    //int index = 0;                               //depthMap[index]
    PVector realWorldPoint;
    
 //   userMap = context.getUsersPixels(SimpleOpenNI.USERS_ALL); //pixels specific to all bodies
    //userMap = context.getUsersPixels(trackUserNum);
 ...
 void trackAllUsersCenterOfMass() {
  ...
  for (int i=0; i<userList.size(); i++) {
    ...
    if ((userList.size() > 1)  && (userList.size() < 4)) {
      ...
      context.getCoM(userId, positionA);
      if (i+1 >= userList.size()) {
       context.getCoM(1, positionB);
      } else {
        context.getCoM(userId+1, positionB);
      }
